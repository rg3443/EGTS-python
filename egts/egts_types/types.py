...
class Simple(EGTSField):
    ...
    def _input_casts(self):
        ...
        return {
            ...
            str: self._unicode_cast,
            bool: self._bool_cast,
            ULong: self._long_cast,
    ...
    def _long_cast(self, value):
        ...
        """
        raise NotImplementedError("Can't cast ULong to class {}".format(type(self)))
...
class LongStored(Simple):
    ...
    def _string_cast(self, value):
        ...
        """
        if value[0:2] == '0x':
            self._value = ULong(value, 16)
        elif value[0:2] == '0b':
            self._value = ULong(value, 2)
        else:
            self._value = ULong(value)
            ...
            for field in self.fields:
                ...
                if isinstance(field, ArrayOfType):
                    if isinstance(item, int):
                        return field[item]
                    for i in range(0, field.quantity):
                        try:
                            return field[i][item]
    ...
    def _string_cast(self, value):
        ...
        if issubclass(self._type, Simple):
            if value[0:2] == '0x':
                ...
                index = 0
                for i in range(2, len(value), size):
